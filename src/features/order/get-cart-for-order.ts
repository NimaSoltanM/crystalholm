...
export const getCartForOrder = createServerFn({
  method: 'GET',
})
...
export const createOrder = createServerFn({
  ...
  .handler(async ({ data }) => {
    try {
      ...
      const { items: cartItemsData } = await getCartForOrder({
        data: data.userId,
      });
      ...
      const orderItemsPromises = cartItemsData.map(async (item) => {
        let enhancedOptions:
          | {
        ...
        if (item.selectedOptions) {
          const optionDetails = await Promise.all(
            item.selectedOptions.map(async (opt) => {
